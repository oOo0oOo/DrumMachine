...
class Model(object):
    def __init__(self,
        ...
        train_step = tf.train.AdamOptimizer(learning_rate=0.00001).minimize(cost)
    ...
    def train_epoch(self, inputs, targets, batch_size = 6, n_epochs = 1):
        l_batches = len(inputs)
        terminal = False

        # batching
        inds = range(l_batches)
        n_batches = int(l_batches / batch_size)
        batch_in = np.array_split(inputs, n_batches)
        batch_ta = np.array_split(targets, n_batches)
        batches = zip(batch_in, batch_ta)

        losses = []
        for b_in, b_ta in batches:
            s = b_in.shape
            b_in = b_in.reshape(s[0], s[2], s[3])
            b_ta = b_ta.reshape(s[0], s[2])
            loss = self._train(b_in, b_ta)
            losses.append(loss)
    ...
    def load_model(self, model_path):
        # Restore previous checkpoint if possible
        checkpoint = tf.train.get_checkpoint_state(model_path)
        epoch = 0
        if checkpoint:
            try:
                self.saver.restore(self.sess, checkpoint.model_checkpoint_path)
                # parse the last epoch
                p = checkpoint.model_checkpoint_path
                dash_ind = p.rfind('-')
                epoch = int(p[dash_ind + 1:])
    ...
    def save_model(self, model_path, step = 0):
        self.saver.save(self.sess, model_path + 'epoch', global_step = step)
